...
void buildAndSaveParseTree(const vector<Token>& tokens) {
    ...
    cout << "Parse tree saved to 'PARSETREE2.dot'. Render using Graphviz: dot -Tpng PARSETREE2.dot -o PARSETREE2.png\n";
}

int main() {
    // MiniLang test case -> invalid syntax
    string sourceCode = R"(
        int x=y+z*z
    )";
